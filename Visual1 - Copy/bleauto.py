...
filename_L = "dataL.csv"
filename_R = "dataR.csv"
...
async def main():
    ...
    def handle_rx(index, _, data: bytearray):
        ...
        if len(data_rows) > 0 and len(data_rows[0]) > 0:
            if data_rows[0][0] == "FSR_IMU_L":  # Access the first element of the first row
                save_data_to_file(filename_L, data_rows)
            if data_rows[0][0] == "FSR_IMU_R":  # Access the first element of the first row
                save_data_to_file(filename_R, data_rows)
